...
function updateAttendanceDate(date){
  ...
  el.innerHTML=agents.map(name=>{
    ...
    const breakTime=recs.filter(r=>r.status==='Away').map(r=>r.duration).join(", ")||'0:00';
    const onlineH=recs.filter(r=>r.status==='Online').length+' hours';
    const chats=Math.floor(Math.random()*100)+20;
    const initials=name.split(' ')[0][0]+(name.split(' ')[1]?name.split(' ')[1][0]:'');
    return `<div style="padding:16px;border:2px solid #ddd;margin:12px;border-radius:8px;background:white"><div style="display:flex;align-items:center;margin-bottom:12px"><div style="width:50px;height:50px;border-radius:50%;background:#6B2D8B;color:white;display:flex;align-items:center;justify-content:center;font-weight:600;margin-right:12px">${initials}</div><div><div style="font-weight:600;font-size:16px">${name}</div><div style="font-size:12px;color:#666">Agent</div></div></div><div style="font-size:13px;color:#666;margin-bottom:12px"><b>${date}</b></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px"><div><div style="font-size:12px;color:#666">Login</div><div style="font-weight:600;font-size:14px">${login}</div></div><div><div style="font-size:12px;color:#666">Logout</div><div style="font-weight:600;font-size:14px">${logout}</div></div><div><div style="font-size:12px;color:#666">Break</div><div style="font-weight:600;font-size:14px">${breakTime}</div></div><div><div style="font-size:12px;color:#666">Chats</div><div style="font-weight:600;font-size:14px">${chats}</div></div></div><div><div style="font-size:12px;color:#666">Online Hours</div><div style="font-weight:600;font-size:14px;margin-bottom:8px">${onlineH}</div></div><div style="width:100%;height:6px;background:#e0e0e0;border-radius:3px;overflow:hidden"><div style="height:100%;width:${Math.min(recs.length*20,100)}%;background:#6B2D8B"></div></div></div>`;
  }).filter(x=>x).join('');
}
function handleAttendance(input){
  const file=input.files[0];
